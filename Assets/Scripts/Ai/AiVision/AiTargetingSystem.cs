...
public class AiTargetingSystem : MonoBehaviour
{
    ...
    void EvaluateScores()
    {
        ...
        //if (bestMemory != null) //Debug.Log(bestMemory);

    }

    float CalculateScore(AiMemory memory)
    {
        float distanceScore = Normalize(memory.distance, DifficultySettings.datas.FOVGuards) * distanceWeight;
        float angleScore = Normalize(memory.angle, sensor.config.angle) * angleWeight;
